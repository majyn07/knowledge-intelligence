...
export function AnalysisPanel({
  ...
  if (!analysisRecord)
    return (
      <PageSection
        ...
        description="Selecione um atendimento e execute a análise para iniciar a revisão técnica."
      >
        <div className="flex min-h-56 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/20 px-6 py-10 text-center">
          ...
          <h2 className="mt-4 font-semibold">A IA ainda não gerou recomendações</h2>
          <p className="mt-2 max-w-lg text-sm leading-6 text-muted-foreground">
            O próximo passo é analisar o atendimento selecionado. Depois, você poderá avaliar
            evidências e decidir o destino de cada oportunidade.
          </p>
        </div>
      </PageSection>
    );
  ...
  const approved = result.opportunities.filter(
    (opportunity) => opportunity.status === "approved"
  ).length;
  const discarded = result.opportunities.filter(
    (opportunity) => opportunity.status === "discarded"
  ).length;
  const deferred = result.opportunities.filter(
    (opportunity) => opportunity.status === "deferred"
  ).length;
  const pending = result.opportunities.filter(
    (opportunity) => opportunity.status === "proposed"
  ).length;
    ...
    <div className="space-y-8">
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard
            label="Cobertura"
            value={DocumentationStatusLabel[result.classification.documentationStatus]}
          />
          <MetricCard
            label="Confiança da IA"
            value={`${result.confidence.toFixed(0)}%`}
            description={ConfidenceLevelLabel[result.classification.confidenceLevel]}
          />
          <MetricCard
            label="Artigos relacionados"
            value={analysisRecord.relatedArticles.length}
            description="Encontrados na Base de Conhecimento"
          />
          <MetricCard
            label="Decisões pendentes"
            value={pending}
            description={
              pending === 0 ? "Revisão pronta para finalizar" : "Avalie cada oportunidade"
            }
          />
        </div>
      </PageSection>
      <AnalysisEvidence analysis={analysisRecord} />
      <RelatedArticlesPanel articles={analysisRecord.relatedArticles} />
      <PageSection
        title="Decisão humana"
        description="Aprove, descarte, adie ou ajuste cada proposta antes de concluir esta revisão."
      >
        <div className="space-y-3">
          {result.opportunities.map((recommendation) => (
            <RecommendationCard
              ...
              readOnly={analysisRecord.status === "completed"}
              onApprove={() => onOpportunityStatusChange(recommendation.id, "approved")}
              onDiscard={() => onOpportunityStatusChange(recommendation.id, "discarded")}
              onDefer={() => onOpportunityStatusChange(recommendation.id, "deferred")}
      ...
      <PageSection
        title={analysisRecord.status === "completed" ? "Análise concluída" : "Encaminhamento"}
        description={
          analysisRecord.status === "completed"
            ? "As decisões foram registradas e as oportunidades aprovadas seguem para a fila de melhorias."
            : pending > 0
              ? "Conclua todas as decisões para liberar a finalização da análise."
              : "A revisão está completa. Finalize para encaminhar as oportunidades aprovadas ao Plano de Melhorias."
        }
      >
        <div className="flex flex-col gap-5 rounded-xl border border-border/70 bg-muted/20 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          {analysisRecord.status === "completed" ? (
            <Button
              variant="outline"
              render={<Link href="/improvement-plan" />}
              nativeButton={false}
            >
              <FileText className="mr-2 h-4 w-4" />
              Ver no Plano de Melhorias
            </Button>
          ) : (
            <Button disabled={!canFinalize} onClick={onFinalize}>
              <CheckCircle2 className="mr-2 h-4 w-4" />
              Finalizar e enviar ao plano
            </Button>
          )}
        </div>
      </PageSection>
      ...
      <OpportunityEditorDialog
        ...
        onSave={(changes) => {
          if (editingOpportunity) onOpportunityUpdate(editingOpportunity.id, changes);
        }}
      />
    </div>
  );
