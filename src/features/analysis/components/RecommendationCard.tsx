      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  {typeName || "Tipo não definido"}
                </p>
                <StatusBadge variant={getStatusVariant(recommendation.status)}>
        ...
        {!readOnly && (
          <div className="flex shrink-0 flex-wrap items-center gap-2 lg:max-w-56 lg:justify-end">
            {isPending && (
              <>
                <Button size="sm" variant="ghost" onClick={() => onEdit(recommendation)}>
                  <Pencil className="mr-1.5 h-3.5 w-3.5" />
                  Editar
                </Button>
                <Button size="sm" variant="ghost" onClick={() => onDefer(recommendation)}>
                  <Clock3 className="mr-1.5 h-3.5 w-3.5" />
                  Adiar
                </Button>
                <Button size="sm" variant="ghost" onClick={() => onDiscard(recommendation)}>
                  Descartar
                </Button>
                <Button size="sm" onClick={() => onApprove(recommendation)}>
                  Aprovar
