...
/** Alto o bastante para o fio caber num pedido, e o teto da API é 100. */
const POR_PAGINA = 100;

/** Trava de segurança: fio nenhum tem 5 mil mensagens, mas laço sem teto trava. */
...
async function mensagensDoFio(threadId: string): Promise<unknown[]> {
  ...
  do {
    ...
    /*
      ...
      devolveu zero registros com o cursor presente e o fio tinha 38. Parar no
...
async function resolverAtores(brutas: unknown[]): Promise<Map<string, HubSpotActor>> {
  ...
  /*
    Um pedido por fio, não um por mensagem: um fio de 38 mensagens costuma ter
...
export const hubspotConversationService = {
  async byExternalId(externalId: string): Promise<HubSpotConversation> {
    ...
    const fios: unknown = await hubspot.get(
      `/conversations/v3/conversations/threads?${query}`
    );

    const threadIds = items(record(fios).results)
      .map((fio) => text(record(fio).id))
      .filter((id) => id !== "");

    const brutas: unknown[] = [];
    for (const threadId of threadIds) {
      brutas.push(...(await mensagensDoFio(threadId)));
    }
